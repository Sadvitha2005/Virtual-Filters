...
def main_frames(name):
    if name == 'dog':
        ...
        while True:
            ...
            try:
                ...
                captured_frames['dog'] = frame1.copy()
    ...
    if name=='pig':
        ...
        while True:
            ...
            try:
                ...
                # Store the current frame for capture
                captured_frames['pig'] = frame3.copy()
    ...
    if name=='panda':
        ...
        while True:
            ...
            try:
                ...
                # Store the current frame for capture
                captured_frames['panda'] = frame5.copy()
    ...
    if name=='snake': 
        ...
        while 1:
            ...
            # Store the current frame for capture before converting to bytes
            captured_frames['snake'] = frame7.copy()
    ...
    if name == 'spooky':
        _, frame9 = camera.read()
        first_frame = None
        spooky_memory = None  # to create ghost trails

        while True:
            try:
                check, frame9 = camera.read()
                if not check:
                    continue
                    
                gray = cv2.cvtColor(frame9, cv2.COLOR_BGR2GRAY)
                ...
                delta_frame_colored = cv2.cvtColor(delta_frame, cv2.COLOR_GRAY2BGR)

                # Add a ghost trail by blending current frame with previous spooky frame
                if spooky_memory is None:
                    spooky_memory = delta_frame_colored.copy()
                else:
                    spooky_memory = cv2.addWeighted(spooky_memory, 0.7, delta_frame_colored, 0.3, 0)

                spooky_frame = spooky_memory.copy()

                # Add slight red tint
                spooky_frame[:, :, 2] = cv2.add(spooky_frame[:, :, 2], 30)  # Boost red channel

                # Random flickering effect
                if random.random() < 0.05:  # 5% chance each frame
                    spooky_frame = (spooky_frame * 0.5).astype('uint8')  # Darken suddenly

                # Store the current spooky frame for capture
                captured_frames['spooky'] = spooky_frame.copy()
                
                # Encode
                ret, buffer = cv2.imencode('.jpg', spooky_frame)
                frame9 = buffer.tobytes()

                yield (b'--frame9\r\n'
                    b'Content-Type: image/jpeg\r\n\r\n' + frame9 + b'\r\n')

            except Exception as e:
                print(f"Error in spooky filter: {str(e)}", file=sys.stderr)
                _, frame10 = camera.read()
                ret, buffer = cv2.imencode('.jpg', frame10)
                frame10 = buffer.tobytes()
    ...
    if name=='fish':
        ...
        while 1:
            ...
            # Store the current frame for capture after applying all effects but before converting to bytes
            captured_frames['fish'] = frame8.copy()
...
@app.route('/video_feed/6')
def video_feed_spooky():
    print(request.base_url, file=sys.stdout)
    return Response(main_frames('spooky'),
                    mimetype='multipart/x-mixed-replace; boundary=frame')                         
...
def capture_photo():
    # Determine which filter is being used from the referrer
    ...
    if 'dog' in referrer:
        filter_type = 'dog'
    elif 'pig' in referrer:
        filter_type = 'pig'
    elif 'panda' in referrer:
        filter_type = 'panda'
    elif 'snake' in referrer:
        filter_type = 'snake'
    elif 'fish' in referrer:
        filter_type = 'fish'
    elif 'spooky' in referrer:
        filter_type = 'spooky'
    
    if filter_type is None:
        return "Unknown filter type", 400
        
    captured_frame = captured_frames.get(filter_type)
    if captured_frame is None:
        print(f"No {filter_type} frame available yet", file=sys.stderr)
        return f"No {filter_type} frame available yet!", 500
    ...
    try:
        ...
        timestamp = int(time())
        photo_path = f"static/{filter_type}_photo_{timestamp}.jpg"
        
        # Debug print
        print(f"Attempting to save image to: {photo_path}", file=sys.stderr)
        print(f"Captured frame shape: {captured_frame.shape}", file=sys.stderr)
        
        success = cv2.imwrite(photo_path, captured_frame)
        if not success:
            print("Failed to write image file", file=sys.stderr)
            return "Failed to save photo", 500

        if not os.path.exists(photo_path):
            print("File was not created", file=sys.stderr)
            return "Failed to save photo", 500

        print(f"Successfully saved image to {photo_path}", file=sys.stderr)
        ...
        response = Response(
            ...
            mimetype='image/jpeg',
            headers={
                'Content-Disposition': f'attachment;filename={filter_type}_photo.jpg',
        ...
        print(f"Error capturing photo: {str(e)}", file=sys.stderr)
        ...
        try:
            if photo_path and os.path.exists(photo_path):
                os.close(os.open(photo_path, os.O_RDONLY))  # Close any open handles
                os.remove(photo_path)
        except Exception as e:
            print(f"Error cleaning up file: {str(e)}", file=sys.stderr)
